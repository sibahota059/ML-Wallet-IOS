...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    if (myError == nil){
        ...
        self.getHistory = getResponse;
        [self.delegate didFinishLoadingHistory:@"1" andError:@""];
        
    }else{
        
        [self.delegate didFinishLoadingHistory:@"1" andError:myError.localizedDescription];
...
- (void)getUserWalletNo:(NSString *)walleno
{
    ...
    NSString *srvcURL = [[[ServiceConnection alloc] NSgetURLService] stringByAppendingString:@"sendoutTopUpHistory"];
